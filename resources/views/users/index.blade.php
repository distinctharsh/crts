      ...
      <div class="modal-body">
        ...
        <form method="POST" action="{{ old('user_id') ? url('users/' . old('user_id')) : route('users.store') }}" id="userForm">
          ...
          <div class="mb-3 d-none" id="passwordFields">
            <label for="password" class="form-label">Password <span id="passwordHelpText"></span></label>
            <input type="password" class="form-control @error('password') is-invalid @enderror" id="password" name="password" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*])[A-Za-z\d!@#$%^&*]{6,}$" title="Password must be at least 6 characters, contain one uppercase letter, one lowercase letter, one digit, and one special character (!@#$%^&*).">
          </div>
...
$(document).ready(function() {
    ...
    $(document).on('click', '[data-bs-target="#userModal"]', function() {
        ...
        $('#passwordConfirmField').addClass('d-none');
    ...
    $(document).on('click', '.editUserBtn', function() {
        ...
        $('#passwordConfirmField').removeClass('d-none');
